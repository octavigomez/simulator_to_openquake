        ...
        for c, i in enumerate(M_filter):
            ...
            if len(eList_filt) >= 3:
                ...
                for lab in np.unique(labels[labels!=-1]):
                    ...
                    moment_late.append(3e10*np.sum(areas_tris[pList_fil][loc_lab]*dList_fil[loc_slip][loc_lab]))
                    moment_total = 3e10*np.sum(areas_tris[pList_fil]*dList_fil[loc_slip])
                    area_late.append(np.sum(areas_tris[pList_fil][loc_lab]))
                    slip_clusters = dList_fil[loc_slip][loc_lab]
                    slip_clusters_perc = np.max(dList_fil[loc_slip][loc_lab])/np.median(dList_fil[loc_slip])
                ...
                loc_perc = np.array(area_late)<np.percentile(np.array(area_late), 50)
                lab_unique = np.unique(labels[labels!=-1])[loc_perc]
                if len(lab_unique)>1:
                    locas = np.concatenate([np.where(labels[labels!=-1] == i)[0] for i in lab_unique])
                elif len(lab_unique)==1:
                    locas = np.array([np.where(labels[labels!=-1] == i)[0] for i in lab_unique]).flatten()
                else:
                    locas = []
                    #nothing
                labs.append(labels[labels!=-1][locas])
